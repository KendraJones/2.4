...
describe("Employee Manager 1.2", () => {
    ...
    /*  ^So I'm running into an error that seems to be preventing the browser from closing. I'm wondering if something is weird with afterALL? Like maybe it's in the wrong scope? Which 
    seems wrong, but that's my best guess. Also, I keep seeing this error:
    "USB: usb_device_handle_win.cc:1048 Failed to read descriptor from node connection: A device attached to the system is not functioning" in my terminal whenever I run a test
    I've done a little googling and it sounds like this might be a known issue with chromedriver. */
    ...
    describe("handles unsaved, canceled, and saved changes correctly", () => {
        ...
        */
        ...
        expect(
            await (await driver.findElement(nameInput)).getAttribute("Bernice Ortiz")
        ).toBe("Bernice Ortiz");
        });
        ...
        test("A canceled change doesn't persist", async () => {
            ...
            */
            ...
            expect(
                await (await driver.findElement(nameInput)).getAttribute("Phillip Weaver")
            ).toBe("Phillip Weaver");
        });
        ...
        test("A saved change persists", async () => {
            ...
            */
            ...
            // Below is the code to verify that edited name saved. "Test Name" instead of "Bernice Ortiz"
            await driver.findElement(bernice).click();
            expect(
    ...
    describe("handles error messages correctly", () => {
        ...
        test("lets you cancel out of an error message", async () => {
            ...
            */
            ...
            expect(await driver.findElements(nameInput)).toHaveLength(0);
        });
    });
    // Everything (other than the test not ending and the browser not closing) is working great!
});